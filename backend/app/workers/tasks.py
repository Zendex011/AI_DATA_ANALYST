...
def run_csv_ask_task(file_id: str, question: str, include_chart: bool) -> dict:
    db = SessionLocal()
    try:
        return run_csv_ask(db, file_id, question, include_chart)
    finally:
        db.close()


@celery_app.task(name="run_db_ask_task")
def run_db_ask_task(db_id: str, question: str, include_chart: bool) -> dict:
    db = SessionLocal()
    try:
        return run_db_ask(db, db_id, question, include_chart)
    finally:
        db.close()